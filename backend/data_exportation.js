...
const GAZE_DATA_PREFIX = "gaze_data_"
const RAW_GAZE_DATA_DIRECTORY = "raw_gaze_data/"
...
exports.get_gaze_data = function (participantId) {
  const gazeDataFile = RAW_GAZE_DATA_DIRECTORY + GAZE_DATA_PREFIX + participantId
...
exports.get_many_gaze_data = function (participantIds) {
  let output = []
  participantIds.map((item, index) => { output.push(exports.get_gaze_data(item)) })
  if (output.length <= 0) 
    return null
  else return output
}
...
function formatDateTime(t) {
  ...
  return (
    d.getFullYear() + '-' + fillZero(d.getMonth() + 1) + '-' + fillZero(d.getDate())
    + '_' + fillZero(d.getHours()) + '-' + fillZero(d.getMinutes())
  )
}

function escapeCSV(term) {
  if (term.includes(',') || term.includes('"')) {      //If either is in the term the field must be enclosed in double quotes
    if (term.includes('"'))       //If it contains double quotes the double quotes must be wrapped with more double quotes
      term = term.replace(/"/g, '""')
    term = '"' + term + '"'
  }
  return term
...
exports.save_to_csv = async function (p, seperator) {
  ...
  await Promise.all(p.linesOfData.map(async (line, index) => {
    ...
    var comments = await (ObserverMessages.find({
      participantId: p._id,
      taskId: line.taskId,
