...
const Header = props => {
  ...
  const getModeButtons = () => {
    const activeColor = props.theme.palette.secondary.main + "66"
    let modeButtons = []
    modeButtons.push(
    ...
    modeButtons.push(
      <Button key="/Observe"
        ...
        style={{
          display: 'flex', position: 'relative', flexGrow: 1, minWidth: 10, maxWidth: 100, width: 0, height: '100%',
          backgroundColor: props.history.location.pathname.includes("/Observe") ? activeColor : ""
        }}>
    ...
    return (
      <AppBar style={{
        ...
        <Toolbar variant="dense" style={{ padding: 0, display: 'flex', flexDirection: 'row', position: 'relative', width: '100%', height: '100%' }}>
          <div style={{
            display: 'flex', position: 'relative', flexShrink: 1, height: '100%', cursor: 'default',
