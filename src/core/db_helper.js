...
class db_helper {
  ...
  */
  ...
  getTaskWithID(id, callback) {
    axios.post("/api/getTaskWithID", { id: id })
      .then( response => {
        if (response.status === 200)
  ...
  addTaskToDb(dbQuestionObject, callback) {
    axios.post("/api/addTask", { message: JSON.stringify(dbQuestionObject), })
      .then( response => {
        if (response.status === 200) {
  ...
  getManyTaskWithIDs(ids, callback) {
    axios.post("/api/getManyTaskWithIDs", { ids: ids })
      .then( response => {
        if (response.status === 200)
          callback(response.data.tasks)
  ...
  */
  ...
  getTaskSetWithID(id, callback) {
    axios.post("/api/getTaskSetWithID", { id: id })
      .then( response => {
        if (response.status === 200)
...
*/
  ...
  */
  ...
  async uploadImage(fileName, image, config, name, callback) {
    await axios.post("/api/uploadImage", image, name, config)
