...
class CollapsableContainer extends Component {
  ...
  render() {
    ...
    const buttonIcon = this.state.open ? <ArrowDropDown fontSize="large" /> : <ArrowDropUp fontSize="large" className="rotateIcon" />;
    const headerComponents = this.props.hideHeaderComponents ?
      this.state.open ? this.props.headerComponents : null
      : this.props.headerComponents
    const headerHeight = this.props.headerHeight ? this.props.headerHeight : 50
    const titleStyles = this.props.titleWidth ? this.props.titleWidth : { width: 55 }
    const titleVariant = this.props.titleVariant ? this.props.titleVariant : "h6"
    const animationTime = this.dropping ? 0 : "auto";
