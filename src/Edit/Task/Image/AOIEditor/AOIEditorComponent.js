...
class AOIEditorComponent extends Component {
  ...
  */
  ...
  onCloseAOINameDialog(name, numberSufficentFixation) {
    if (name !== "") {
      try {
    ...
    this.tempAOI = new AOI()
    this.setState({ openAOINameDialog: false })
  }
...
*/
  ...
  finishDrawingPolygon() {
    if (this.tempAOI.boundingbox.length < 3) {
      alert("Cannot create a polygon with less than 3 points")
      return
    }
    this.setState({ openAOINameDialog: true })
  }
