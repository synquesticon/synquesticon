...
const EditSetList = props => {
  ...
  if (props.taskListObjects.length === 0)
    return (
      <Typography variant='h5' color='textPrimary' style={{ opacity: 0.5, padding: 5, marginLeft: 5 }}>
  ...
  const collapsableContent = props.taskListObjects.map((item, index) => {
    ...
    return (
      <Draggable
        key={item._id + "setListId" + index}
        draggableId={item._id + "setListId" + index}
        index={index}
        shouldRespectForceTouch={false}
      >
        {(provided, snapshot) => (
          <div className={"editSetListItem "}
            key={item._id + "setListId_item" + index}
          ...
          >
            <EditSetListItem
              index={index}
              item={item}
              content={listUtils.getTaskContent(item)}
              componentDepth={0}
              handleDrop={props.dragDropCallback}
              removeCallback={props.removeTaskCallback}
              moveTaskCallback={props.moveTaskCallback}
            />
          </div>
        )}
      </Draggable>
